...
void Server::cleanup_connection(Connection* conn)
{
    ...
    if (conn->state == CGI_RUNNING && conn->cgi_fd != -1)
    {
        epoll_ctl(epoll_fd, EPOLL_CTL_DEL, conn->cgi_fd, NULL);
        close(conn->cgi_fd);
        conn->cgi_fd = -1;
    }
...
void Server::handle_client(Connection* conn)
{
    ...
    const Location* matchedLocation =
        conn->serverConfig->matchLocation(request.getPath());
    ...
    if (CGIHandler::isCGI(request.getPath()))
    {
        std::string res =
            CGIHandler::handle(conn, request, *conn->serverConfig, epoll_fd);
        ...
        _clientWriteBuffers[conn->fd] = res;
        struct epoll_event ev;
        ev.events = EPOLLOUT;
        ev.data.ptr = conn;
...
void Server::handle_cgi(Connection* conn)
{
    ...
    while (true)
    {
        ssize_t bytes = read(conn->cgi_fd, buffer, sizeof(buffer));

        if (bytes > 0)
        {
            conn->cgi_buffer.append(buffer, bytes);
            continue;
        }
    ...
    std::string response = CGIHandler::buildResponseFromCGI(conn->cgi_buffer);
...
void Server::run()
{
    ...
    while (g_keepRunning)
    {
        ...
        for (int i = 0; i < nfds; i++)
        {
            ...
            else if (conn->state == CGI_RUNNING && (events[i].events & EPOLLIN))            {
                handle_cgi(conn);
            }
